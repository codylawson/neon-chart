$(document).ready(function() {
  ...
  */
  ...
  function _wireAtmWinTooltip() {
    ...
    $.each(atmosphericWindows, function(atmoWin, atmItem) {
  ...
  $.each(atmosphericWindows, function(index, item) {
    chart.regions.add({
      axis: 'x',
  ...
  $.each(spectrumRanges, function(index, item) {
    chart.regions.add({
      axis: 'x',
      start: item.start,
      end: item.end,
  ...
  $.getJSON('js/dataBands.json', function(data) {
    ...
    $.each(data, function(index, item) {
      chart.load({
        columns: [
          ['x'].concat(xAxisLabels),
          [index].concat(_nullAtmWinVals(item))
        ],
        type: 'spline'
      });
    });
